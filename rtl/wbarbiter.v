...
//
//	1. When no one requests the bus, the last user maintains access to the
//		bus, and able to place accesses on the bus in combinatorial
//		logic only.
//	2. If the other master requests the bus (asserts cyc), and the bus is
//		idle, ownership transfers and the other master is granted the
//		ability to use the bus.
//	3. Bus grants last as long as the 'cyc' line is high.
//	4. Once 'cyc' is dropped, the bus remains owned until a master asserts
//		'cyc', at which point arbitration as in step 1 above is
//		repeated.
//
//
...
module	wbarbiter(i_clk, 
	...
	reg	r_a_owner;
	initial	r_a_owner = 1'b1;	// A starts out owning the bus
	always @(posedge i_clk)
		if ((i_a_cyc)&&(!i_b_cyc))
			r_a_owner <= 1'b1;
		else if ((i_b_cyc)&&(!i_a_cyc))
			r_a_owner <= 1'b0;
		// else ... ownership doesn't change
	...
	// irrelevant.
	//
	// The o_stb line assumes i_x_stb will _never_ be true unless i_x_cyc
	// is also true.  Nothing is done to check for this.  For i_x_stb
	// to be true while i_x_cyc is false is an error.
	//
